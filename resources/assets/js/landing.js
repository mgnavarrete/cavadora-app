...
function ltrFn() {
  ...
  document
    ...
    ?.setAttribute(
      "href",
      "http://127.0.0.1:8000/build/assets/libs/bootstrap/css/bootstrap.min.css"
    );
  html.setAttribute("dir", "ltr");
...
pageLink.forEach((elem) => {
  ...
  const isExternalLink = elem.hasAttribute("data-external-link");
  
  // Solo aplicar scroll suave a enlaces que sean anclas de la misma página (#)
  // y que NO sean enlaces externos marcados
  if (href && href.startsWith("#") && href !== "#" && !isExternalLink) {
  ...
  // Los enlaces externos (como /login) se manejarán normalmente sin preventDefault
});
// section menu active
function onScroll(event) {
  ...
  sections.forEach((elem) => {
    ...
    const isExternalLink = elem.hasAttribute("data-external-link");
    let refElement;
    if (val && val.startsWith("#") && val !== "#" && !isExternalLink) {
